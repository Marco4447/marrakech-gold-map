...
// Credit packs mapping: price_id -> credits
...
serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: "payment",
      success_url: `${req.headers.get("origin")}/shop?success=true&credits=${CREDIT_PACKS[priceId]}`,
      cancel_url: `${req.headers.get("origin")}/shop?canceled=true`,
      metadata: {
        user_id: user.id,
        credits: String(CREDIT_PACKS[priceId]),
      },
    });
