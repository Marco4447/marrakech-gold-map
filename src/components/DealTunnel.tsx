...
export default function DealTunnel({ open, onOpenChange, placeName }: DealTunnelProps) {
  ...
  const displayName = profile?.full_name || (t("free") === "Free" ? "Guest" : "Invité");
              ...
              <div className="px-6 pb-8">
                {step === "email" ? (
                  <>
                    ...
                    <div className="bg-gold/5 border border-gold/15 rounded-xl p-3 mb-5 space-y-1.5">
                      ...
                      <ul className="text-xs text-muted-foreground space-y-1">
                        <li>🔑 {t("deal_communityAccess")} <span className="text-gold">Weshkech</span></li>
                        <li>🎟️ {t("deal_qrCode")} {placeName}</li>
                      </ul>
                    </div>
                    <div className="space-y-2 mb-5">
                      <label className="text-xs text-muted-foreground uppercase tracking-wider font-medium flex items-center gap-1.5"><Mail className="w-3 h-3" /> {t("deal_yourEmail")}</label>
                      <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="votre@email.com" className="w-full bg-surface border border-border rounded-xl px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-gold/30 focus:border-gold/50 transition-all" onKeyDown={(e) => e.key === "Enter" && handleSubmit()} />
                    </div>
                  ...
                  <div className="text-center py-4">
                    <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", damping: 15 }} className="w-16 h-16 rounded-full bg-gold/15 flex items-center justify-center mx-auto mb-4"><Check className="w-8 h-8 text-gold" /></motion.div>
                    <h2 className="font-display text-xl font-semibold text-foreground mb-1">{t("deal_passTitle")}</h2>
                    ...
                    <AnimatePresence>
                      {showFeedback && !feedbackSent && (
                        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="mb-3 space-y-2">
                          <textarea value={feedbackText} onChange={(e) => setFeedbackText(e.target.value)} placeholder={t("deal_feedbackPlaceholder")} rows={3} className="w-full bg-surface border border-border rounded-xl px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-gold/30 focus:border-gold/50 transition-all resize-none" />
                          <button onClick={() => { if (!feedbackText.trim()) return; setFeedbackSent(true); setShowFeedback(false); }} disabled={!feedbackText.trim()} className="w-full bg-gold hover:bg-gold-light disabled:opacity-40 text-primary-foreground font-semibold py-3 rounded-xl transition-all flex items-center justify-center gap-2"><Send className="w-4 h-4" />{t("deal_sendFeedback")}</button>
                        </motion.div>
                      )}
                    </AnimatePresence>
                    {feedbackSent && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-gold/10 border border-gold/20 rounded-xl p-3 mb-3 flex items-center gap-2 justify-center"><Check className="w-4 h-4 text-gold" /><span className="text-sm text-gold font-medium">{t("deal_thanksFeedback")}</span></motion.div>)}
                    <button onClick={handleClose} className="w-full bg-surface hover:bg-surface-elevated text-foreground font-medium py-3 rounded-xl transition-colors border border-border">{t("close")}</button>
                  </div>
                )}
              </div>
            </div>
