...
export default function ProfilPage({ onOpenAdmin, onClose }: ProfilPageProps) {
  ...
  const fetchFavorites = useCallback(async () => {
    ...
    if (allNeededIds.length > 0) {
      batch2.push(
        Promise.resolve(supabase.from("vibes").select(VIBE_COLUMNS).in("id", allNeededIds).order("created_at", { ascending: false }))
      );
    } else {
    ...
    if (checkinPlaceIds.length > 0) {
      batch2.push(Promise.resolve(supabase.from("places").select("name, image_url, slug").in("id", checkinPlaceIds).limit(50)));
    } else {
      batch2.push(Promise.resolve({ data: [] }));
    }
