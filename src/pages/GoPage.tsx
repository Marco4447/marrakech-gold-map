...
const TESTIMONIALS = [
  { name: "Sophia", text: "J'ai trouvé le meilleur rooftop en 2 min 🔥", flag: "🇫🇷", avatar: "S" },
  { name: "Youssef", text: "Les deals VIP sont incroyables", flag: "🇲🇦", avatar: "Y" },
  { name: "Emma", text: "Indispensable pour sortir à Kech", flag: "🇬🇧", avatar: "E" },
  { name: "Karim", text: "Meilleure app pour découvrir Marrakech", flag: "🇲🇦", avatar: "K" },
  { name: "Julie", text: "On a découvert des pépites grâce à Weshkech", flag: "🇧🇪", avatar: "J" },
];
...
export default function GoPage() {
  ...
  // Trending places for preview cards
  const { data: trendingPlaces } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: vibes } = await supabase
        .from("vibes")
        .select("location, image_url, likes, super_vibes")
      ...
      if (!vibes || vibes.length === 0) {
        // Fallback: get places with images
      ...
      return Object.entries(grouped)
        ...
        .slice(0, 3)
        .map(([name, d]) => ({ name, image: d.image, count: d.count, category: "trending" }));
    },
    staleTime: 5 * 60 * 1000,
  });

  // Recent vibe images for social proof strip
    ...
    <div className="min-h-[100dvh] bg-background relative overflow-hidden flex flex-col">
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center px-5 pt-8 pb-28 max-w-md mx-auto w-full">
        ...
        <motion.div
          ...
          className="inline-flex items-center gap-2 bg-gold/10 border border-gold/20 px-4 py-1.5 rounded-full mb-4"
        >
        ...
        >
          <h1 className="font-display text-[2.2rem] font-black leading-[1.08] tracking-tight">
            <span className="text-gold">🔥 What's trending</span>
            <br />
            <span className="text-foreground">in Marrakech tonight</span>
          </h1>
          <p className="text-muted-foreground text-sm mt-2.5 leading-relaxed max-w-[300px] mx-auto">
            Discover the restaurants, rooftops and clubs people are visiting <span className="text-gold font-medium">right now</span>.
          </p>
        </motion.div>
        ...
        >
          {liveVibes != null && (
            <div className="flex items-center gap-1.5 bg-destructive/10 border border-destructive/20 rounded-full px-3 py-1.5">
              ...
              <span className="text-xs font-bold text-destructive-foreground">{liveVibes} live now</span>
            </div>
          )}
          {usersCount != null && (
        ...
        {trendingPlaces && trendingPlaces.length > 0 && (
          ...
          >
            <div className="flex items-center gap-1.5 mb-2.5">
              ...
              <span className="text-[10px] font-bold text-gold uppercase tracking-wider">Trending now</span>
            </div>
            <div className="space-y-2">
              {trendingPlaces.map((spot, i) => (
                <motion.div
                  ...
                  className="flex items-center gap-3 bg-card/70 backdrop-blur-md border border-border rounded-2xl p-2.5 cursor-pointer active:scale-[0.98] transition-transform"
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-[10px] text-gold font-medium flex items-center gap-0.5">
                        ...
                        {spot.count * 12 + Math.floor(Math.random() * 50)} checking this
                      </span>
                    </div>
                  </div>
        ...
        >
          ...
          See the hottest spots
          <ArrowRight className="w-5 h-5" />
        </motion.button>

        {/* ===== SOCIAL PROOF — People discovering tonight ===== */}
        <motion.div
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <div className="flex items-center gap-1.5 mb-2.5">
            ...
            <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">People discovering Marrakech tonight</span>
          </div>
            ...
            <div className="flex gap-1.5 overflow-x-auto no-scrollbar mb-3">
              {recentVibeImages.slice(0, 6).map((v, i) => (
                <div
                  ...
                  style={{ filter: i > 2 ? "blur(4px)" : "none", opacity: i > 2 ? 0.6 : 1 }}
                >
        ...
        >
          <button
            onClick={() => {
              const shareData = {
                title: "Weshkech",
                text: "🔥 Check out what's trending in Marrakech tonight!",
          ...
          <button
            onClick={handleDiscover}
            className="py-3 rounded-2xl bg-gold/5 border border-gold/20 text-gold text-xs font-semibold flex items-center justify-center gap-1.5 active:scale-[0.97] transition-transform"
          >
            🏆 Unlock VIP access
          </button>
        </motion.div>
        ...
        <p className="text-[10px] text-muted-foreground mt-2 text-center">🇲🇦 Made in Marrakech</p>
      </div>
